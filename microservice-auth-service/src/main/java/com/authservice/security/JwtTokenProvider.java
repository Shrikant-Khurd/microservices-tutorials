...
public class JwtTokenProvider {
	...
	public String generateJwtToken(User user) {
		Set<UserRole> rolesSet = new HashSet<>(user.getRoles());
		List<UserRoleDto> rolesList = rolesSet.stream().map(userRole -> new UserRoleDto(userRole.getRoleName()))
	...
	public boolean validateToken(String token) {
		 ...
		 * true; } catch (EmailAlreadyExistException ex) { throw new
		 * EmailAlreadyExistException(
		 * messageSource.getMessage("api.error.user.already.registered", null,
		 * Locale.ENGLISH)); }
		 */
